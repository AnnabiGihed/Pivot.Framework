...
/// Author      : Gihed Annabi
/// Date        : 01-2026
/// Modified    : 03-2026 — Added aggregate-aware overload for event store enrichment.
...
public interface IDomainEventPublisher
{
	...
	/// <summary>
	/// Publishes the specified domain event with aggregate metadata for event store enrichment.
	/// The aggregate context (type, ID, version) is captured in the event envelope.
	/// </summary>
	/// <param name="domainEvent">The domain event to publish.</param>
	/// <param name="aggregate">The aggregate root that raised this event.</param>
	/// <param name="cancellationToken">A cancellation token to observe while waiting for the operation to complete.</param>
	/// <returns>A <see cref="Result"/> indicating success or failure of the publish operation.</returns>
	Task<Result> PublishAsync(IDomainEvent domainEvent, IAggregateRoot aggregate, CancellationToken cancellationToken);
