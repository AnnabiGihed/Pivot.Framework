...
public sealed class RabbitMQTopologyManager : IRabbitMQTopologyManager, IAsyncDisposable
{
    ...
    #region Constructor
    public RabbitMQTopologyManager(IOptions<RabbitMQSettings> settings, IOptions<TopologyOptions> topologyOptions, ILogger<RabbitMQTopologyManager> logger)
	{
    ...
    private async Task DeclareBindingAsync(ExchangeBinding binding, CancellationToken ct)
	{
        ...
        #region 1. Declare the main exchange (topic)
        await _channel.ExchangeDeclareAsync(binding.Exchange, binding.ExchangeType, durable: true, autoDelete: false, arguments: null, cancellationToken: ct);

		_logger.LogDebug("Declared exchange: {Exchange} ({Type})", binding.Exchange, binding.ExchangeType);
        #endregion

        #region 2. Build queue arguments
        ...
        #region 3. Declare DLX and DLQ if enabled
        if (binding.EnableDeadLetterQueue)
		{
			var dlxName = $"{binding.Exchange}.dlx";
			var dlqName = $"{binding.Queue}.dlq";

            #region Declare dead-letter exchange
            await _channel.ExchangeDeclareAsync(dlxName, ExchangeType.Direct, durable: true, autoDelete: false, arguments: null, cancellationToken: ct);
            #endregion

            #region Declare dead-letter queue
            var dlqArgs = new Dictionary<string, object?>();
			if (binding.UseQuorumQueue)
				dlqArgs["x-queue-type"] = "quorum";

			await _channel.QueueDeclareAsync(dlqName, durable: true, exclusive: false, autoDelete: false, arguments: dlqArgs, cancellationToken: ct);

			await _channel.QueueBindAsync(dlqName, dlxName, binding.Queue, arguments: null, cancellationToken: ct);
            #endregion

            #region  Set DLX on the main queue
        ...
        #region 4. Declare retry delay queue if retry is enabled
        if (binding.MaxRetryCount > 0 && binding.RetryDelayMs > 0)
		{
			...
			await _channel.QueueDeclareAsync(retryQueueName, durable: true, exclusive: false, autoDelete: false, arguments: retryArgs, cancellationToken: ct);
		...
		_logger.LogDebug("Declared queue: {Queue}, bound to {Exchange} with key {RoutingKey}", binding.Queue, binding.Exchange, binding.RoutingKey);
	}
    #endregion
    ...
    #region IRabbitMQTopologyManager implementation
    /// <inheritdoc />
