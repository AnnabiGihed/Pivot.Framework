...
namespace Pivot.Framework.Infrastructure.Persistence.EntityFrameworkCore.MultiContext;

/// <summary>
/// Author      : Gihed Annabi
/// Date        : 03-2026
/// Purpose     : Read/write database separation helper for multi-context deployments.
///              Allows registering a write DbContext (primary) and a read DbContext (replica)
///              with separate connection strings for CQRS read/write split.
/// </summary>
public static class ReadWriteDbContextExtensions
{
	/// <summary>
	/// Registers separate read and write DbContexts for CQRS read/write DB separation.
	/// The write context connects to the primary database; the read context connects to
	/// a read replica.
	/// </summary>
	...
	public static IServiceCollection AddReadWriteDbContexts<TWriteContext, TReadContext>(
		...
		where TReadContext : DbContext
	{
		...
		{
			options.UseSqlServer(readConnectionString, sql => sql.UseQuerySplittingBehavior(QuerySplittingBehavior.SplitQuery));
